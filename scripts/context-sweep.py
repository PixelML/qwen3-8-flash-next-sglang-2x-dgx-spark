#!/usr/bin/env python3
"""Qwen3.8-Flash-Next 2x DGX Spark prompt-length sweep, thinking on vs off.
...
def read_metrics() -> dict:
    """Cumulative SGLang counters. Returns {} if /metrics is unavailable."""
    out = {"generation_tokens_total": 0.0, "spec_verify_calls_total": 0.0,
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as r:
            for raw in r:
                ...
                if line.startswith("#"):
                    continue
                for key in ("generation_tokens_total", "spec_verify_calls_total",
                            "prompt_tokens_total"):
...
def spec_delta(before: dict, after: dict) -> dict:
    ...
    d_ver = after.get("spec_verify_calls_total", 0) - before.get("spec_verify_calls_total", 0)
    return {
        "gen_tokens_delta": round(d_gen, 2),
        "verify_calls_delta": round(d_ver, 2),
        "accepted_tokens_per_pass": round(d_gen / d_ver, 4) if d_ver > 0 else None,
    }
...
def flush_cache() -> bool:
    ...
    """
    try:
        req = urllib.request.Request(f"{BASE}/flush_cache", headers=HDRS, method="POST")
